...
class AssessmentAgent(BaseOrchestrationAgent):
    agent_name = AgentName.ASSESSMENT
    artifact_key = "assessment_resources"
    ...
    async def run(self, context: AgentRunContext) -> AgentResult:
        ...
        items = [
            ...
            if item.get("recommendation_type") in {"quiz", "flashcards"}
        ]
        return AgentResult(
            agent_name=self.agent_name,
            status=RunStatus.COMPLETED,
            summary=f"Prepared {len(items)} assessment resources through registered tools.",
